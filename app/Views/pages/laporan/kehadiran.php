        ...
        <form method="get" class="flex flex-wrap items-end gap-4">
            <div>
                ...
                <select name="class_id" id="class_id"
                    class="block w-full mt-1 py-2 px-3 text-sm rounded-lg border-gray-300" required>
            ...
            <div>
                ...
                <select name="month" id="month" class="block w-full mt-1 py-2 px-3 text-sm rounded-lg border-gray-300">
                    <?php for ($m = 1; $m <= 12; $m++): ?>
                        <option value="<?= $m ?>" <?= $selected_month == $m ? 'selected' : '' ?>> <?= $bulanIndonesia[$m] ?>
                        </option><?php endfor; ?>
                </select>
            </div>
            <div>
        ...
        <div class="w-full overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                    ...
                    <tr class="text-xs font-semibold text-center text-gray-500 uppercase border-b bg-gray-50">
                        ...
                        <?php foreach ($dateHeaders as $date): ?>
                            <th class="px-3 py-3 min-w-[60px]">
                                <?php $month_num = date('n', strtotime($date));
                                echo date('d', strtotime($date)) . ' ' . substr($bulanIndonesia[$month_num], 0, 3); ?>
                            </th>
                        <?php endforeach; ?>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="1" class="text-center py-8 text-gray-500">Silakan pilih kelas untuk menampilkan
                                laporan.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
        ...
        <div @click.away="close()" class="bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6">
            <div class="flex justify-between items-start border-b pb-3">
                ...
                <div class="flex items-center space-x-2 flex-shrink-0">
                    <a :href="'<?= site_url('admin/laporan/siswa/') ?>' + studentId"
                        class="px-3 py-1 text-xs font-medium text-white bg-sky-600 rounded-md hover:bg-sky-700">Laporan
                        Lengkap</a>
            ...
            <div class="grid grid-cols-4 gap-4 my-4 text-center">
                ...
                <div class="p-2 bg-yellow-50 rounded-lg">
                    <p class="text-2xl font-bold text-yellow-600" x-text="summary.sakit"></p>
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y">
                        ...
                        <template x-if="attendanceData.length === 0">
                            <tr>
                                <td colspan="4" class="text-center text-gray-500 py-8">Tidak ada catatan kehadiran pada
                                    tahun ajaran ini.</td>
                            </tr>
                        </template>
                    </tbody>
